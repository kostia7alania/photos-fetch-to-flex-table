...
export default {
  ...
  components: { "animated-number": () => import("animated-number-vue") },
  data() {
    return {
      perPageModel: 25,
  ...
  methods: {
    ...
    perPageHandler() {
      this.$emit("perPageEmit", this.perPageModel);
    },
    nav_handler(e) {
      let nextPage = this.nav_computed.filter(f => e.target.title == f.title)[0]
        .gotoPage;
  ...
  computed: {
    pages: function() {
      return Math.ceil(this.Found / this.perPage);
    },
    ...
    */
      let nav = [
        ...
        .map(e => {
          if (this.Page + 1 < this.pages) {
            if (e.class == "navigate_next")
              return {
                title: e.title,
                class: e.class,
                gotoPage: this.Page + 1
              };
            if (e.class == "navigate_end")
              return { title: e.title, class: e.class, gotoPage: this.pages };
          }
          if (this.Page != 0) {
            if (e.class == "navigate_prev")
              return {
                title: e.title,
                class: e.class,
                gotoPage: this.Page - 1
              };
            if (e.class == "navigate_begin")
              return { title: e.title, class: e.class, gotoPage: 0 };
          }
        })
